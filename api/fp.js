...
// ── PROBABILITY: P(ball in play | any pitch) ──────────────────────────────
// = pitcher Strike% × batter (Swing/Strike × Contact/Swing × InPlay/Contact)
// Batter inPlayGame already represents the full chain per pitch faced
// So: prob = (pitcherStrike% / 100) × (batterInPlayGame / batterStrike%)
// This correctly accounts for pitcher strike rate
function calcProb(pData, bData) {
  if (!pData || !bData) return null;
  const pStrike = pData.strikePct;
  const bStrike = bData.strikePct;
  const bInPlay = bData.inPlayGame; // already full chain per pitch faced (strikeFaced% × swing × contact × inPlay)
  if (!pStrike || !bStrike || bInPlay == null) return null;

  // bInPlayGame = bStrike% × swing% × contact% × inPlay% / 10^6 × 100
  // = batter's P(in play | pitch faced by batter)
  // Adjust for pitcher's strike rate vs batter's experienced strike rate:
  // prob = (pStrike / bStrike) × bInPlayGame / 100
  // Capped at reasonable bounds
  const ratio = bStrike > 0 ? pStrike / bStrike : 1;
  const prob = Math.min(0.5, Math.max(0, (ratio * bInPlay) / 100));
