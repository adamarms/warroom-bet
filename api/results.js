...
function calcStats(plays, market) {
  ...
  active.forEach(p => {
    ...
    // Use actual units wagered if available, otherwise derive from odds
    if (units != null && units > 0) {
      riskSum += units;
    } else if (odds != null) {
      riskSum += odds < 0 ? Math.abs(odds / 100) : 1;
    } else {
      riskSum += 1;
    }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Group by date for chart data — cumulative total + individual bet results
    const byDate = {};
    scoredPlays.forEach(p => {
      if (!p.date) return;
      if (!byDate[p.date]) byDate[p.date] = { ml: 0, ou: 0, rl: 0, bets: [] };
      if (p.mlPL != null && p.mlTier && p.mlTier !== "No Play (final)") {
        byDate[p.date].ml += p.mlPL;
        byDate[p.date].bets.push(Math.round(p.mlPL * 100) / 100);
      }
      if (p.ouPL != null && p.ouTier && p.ouTier !== "No Play (final)") {
        byDate[p.date].ou += p.ouPL;
        byDate[p.date].bets.push(Math.round(p.ouPL * 100) / 100);
      }
      if (p.rlPL != null && p.rlTier && p.rlTier !== "No Play (final)") {
        byDate[p.date].rl += p.rlPL;
        byDate[p.date].bets.push(Math.round(p.rlPL * 100) / 100);
      }
    });

    const sortedDates = Object.keys(byDate).sort();
    let cumML = 0, cumOU = 0, cumRL = 0;
    const chartData = sortedDates.map(date => {
      cumML += byDate[date].ml;
      cumOU += byDate[date].ou;
      cumRL += byDate[date].rl;
      return {
        date,
        ml:    Math.round(cumML * 100) / 100,
        ou:    Math.round(cumOU * 100) / 100,
        rl:    Math.round(cumRL * 100) / 100,
        total: Math.round((cumML + cumOU + cumRL) * 100) / 100,
        bets:  byDate[date].bets   // individual bet P/L for bar chart
      };
    });
