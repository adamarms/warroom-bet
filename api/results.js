...
const PL = {
  ...
  // New explicit risked columns (AX=49, AY=50, AZ=51)
  MLRisked:49, OURisked:50, RLRisked:51
};
...
function parseRow(r) {
  return {
    ...
    ouCLV:       pn(r[PL.OUCLV]),
    rlCLV:       pn(r[PL.RLCLV]),
    // Explicit risked units — written by scoreResults, null for historical rows
    mlRisked:    pn(r[PL.MLRisked]),
    ouRisked:    pn(r[PL.OURisked]),
    rlRisked:    pn(r[PL.RLRisked])
  };
}
...
function calcStats(plays, market) {
  ...
  const riskedKey = market + "Risked";
  const oddsKey   = market === "ml" ? "mlOdds" : market === "ou" ? "ouJuice" : "rlOdds";
  const unitsKey  = market === "ml" ? "mlUnits" : null;

  // Only plays with a tier AND a completed result
  const active = plays.filter(p => {
    const tier   = p[tierKey];
    const result = p[resultKey];
    return tier && tier !== "" && tier !== "No Play (final)"
      && result && result !== "";
  });
  ...
  active.forEach(p => {
    ...
    // ROI denominator: prefer explicit risked column (new plays)
    // Fall back to inference for historical rows that predate the new columns
    if (p[riskedKey] != null) {
      riskSum += p[riskedKey];
    } else if (odds != null) {
      // Legacy inference: same logic as scoreResults for backward compat
  ...
  return {
    ...
    plUnits: Math.round(plUnits * 10) / 10,
    clvAvg:  clvCount > 0 ? Math.round(clvSum / clvCount * 10) / 10 : null,
    roi:     roi != null ? Math.round(roi * 10) / 10 : null,
    riskSum: Math.round(riskSum * 1000) / 1000
  };
}
...
module.exports = async (req, res) => {
  ...
  try {
    // Extend range to AZ (col 52) to capture new risked columns
    const rows = await fSheet("Play Log!A1:AZ2000");
    if (rows.length <= 1) { res.status(200).json({ plays:[], stats:{}, ts:Date.now() }); return; }

    const allPlays = rows.slice(1).filter(r => r[0] && r[1]).map(parseRow);
    const scoredPlays = allPlays.filter(p =>
      p.mlTier || p.ouTier || p.rlTier || p.mlResult || p.ouResult || p.rlResult
    );
    ...
    const totalPL    = Math.round((mlStats.plUnits + ouStats.plUnits + rlStats.plUnits) * 10) / 10;
    const totalRisk  = Math.round(((mlStats.riskSum||0) + (ouStats.riskSum||0) + (rlStats.riskSum||0)) * 1000) / 1000;
    ...
    const totalROI   = totalRisk > 0 ? Math.round(totalPL / totalRisk * 1000) / 10 : null;

    // Chart: individual ML plays only, chronological
    ...
    const chartData = mlPlays.map((p, i) => {
      cumML += p.mlPL || 0;
      return {
        i, date: p.date, gameId: p.gameId,
        away: p.away, home: p.home, team: p.mlTeam,
        odds: p.mlOdds,
        pl:   Math.round((p.mlPL || 0) * 10) / 10,
        cumML: Math.round(cumML * 10) / 10,
        result: p.mlResult
      };
    });

    res.status(200).json({
      plays: scoredPlays.reverse(),
      stats: {
        total: { plays:totalPlays, wins:totalWins, losses:totalLoss, plUnits:totalPL, riskSum:totalRisk, roi:totalROI },
        ml: mlStats, ou: ouStats, rl: rlStats
      },
      chartData,
      ts: Date.now()
    });
