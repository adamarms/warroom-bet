...
function calcStats(plays, market) {
  ...
  active.forEach(p => {
    ...
    // Risk for ROI: flat 1u risk
    if (odds != null) {
      riskSum += odds < 0 ? Math.abs(100 / odds) : 1;
    } else {
      riskSum += 1;
    }
  });
  ...
  return {
    ...
    roi:     riskSum > 0 ? Math.round(plUnits / riskSum * 1000) / 10 : null
  };
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Group by date for chart data (cumulative P/L)
    const byDate = {};
    scoredPlays.forEach(p => {
      if (!p.date) return;
      if (!byDate[p.date]) byDate[p.date] = { ml: 0, ou: 0, rl: 0 };
      byDate[p.date].ml += p.mlPL || 0;
      byDate[p.date].ou += p.ouPL || 0;
      byDate[p.date].rl += p.rlPL || 0;
    });
