...
// Fuzzy match logic (same as engine)
function fuzzyMatch(name, warMap) {
  ...
  for (var wk in warMap) {
    var w = warMap[wk];
    var lastName = w.name.split(" ").pop().toUpperCase();
    if (k.endsWith(lastName) && k[0] === w.name[0].toUpperCase()) {
      found = { match: "FUZZY", warName: w.name, reason: "last name + first initial" };
      break;
    }
  }
  return found || { match: "NONE", warName: null };
}
