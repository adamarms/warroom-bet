  ...
  try {
    ...
    Object.values(bestRow).forEach(({ row: r }) => {
      ...
      if (hasML && !gameList.find(g => g.id === gameKey)) {
        ...
        // Apps Script writes "YES" only when MLB StatsAPI returns 9+ official players.
        // This prevents the Vercel API's live MLB StatsAPI call from firing
        // on projected lineups and triggering false-positive confirmations.
  ...
  try {
    ...
    (data.dates || []).forEach(dt => { (dt.games || []).forEach(g => {
      ...
      if (!mlbMap[key] || sp !== "TBD")
          ...
          // NOTE: aC/hC intentionally NOT set here.
          // Lineup confirmation comes from Odds JSON BothConfirmed column only.
          // MLB StatsAPI returns projected lineups early which cause false positives.
  ...
  try {
    ...
    return oddsGames.map(g => {
      ...
      return {
        ...
        // RotoWire "Confirmed Lineup" status is a valid secondary confirmation source
        aC: g.aC || m.aC,
        hC: g.hC || m.hC
      };
    });
