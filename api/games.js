...
// WAR blend: (1 - teamSeasonPct) × projected + actual
// teamPct comes from Team Projections column G (GP/162)
...
function pPW(rows) {
  ...
  for (let i = s; i < rows.length; i++) { const r = rows[i]; if (!r?.[0]) continue;
    ...
    // Key by name+team to handle duplicates (e.g., two Max Muncys)
    const teamKey = n.toUpperCase() + "|" + team;
    const normTeamKey = norm + "|" + team;
    m[teamKey] = entry;
    if (normTeamKey !== teamKey) m[normTeamKey] = entry;
    // Also store name-only keys, but only if no collision
...
function findPlayer(name, warMap, team) {
  ...
  // 1. Exact match on name+team key (handles Max Muncy on LAD vs SAC)
  if (teamUp && warMap[k + "|" + teamUp]) return warMap[k + "|" + teamUp];
  if (teamUp && warMap[norm + "|" + teamUp]) return warMap[norm + "|" + teamUp];
  // 2. Exact match on name only (safe when no duplicate)
  if (warMap[k]) return warMap[k];
  if (warMap[norm]) return warMap[norm];
  // 3. First name + last name + team (handles middle initials)
  ...
  // 4. First name + last name WITHOUT team filter (fallback for unknown team)
  if (teamUp) {
  ...
  // 5. Handle abbreviated first name (T. Grisham, J. Misiorowski) with team
  ...
  // 6. Last resort: last name + first initial
