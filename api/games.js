...
async function fSheet(range) {
  ...
  const u = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(range)}?key=${SHEET_KEY}&valueRenderOption=UNFORMATTED_VALUE&cb=${cb}`;
...
async function fOdds() {
  try {
    ...
    rows.forEach(r => {
      ...
      const pullDate = String(r[0]).slice(0, 10); const rawTime = r[1];
      // Normalize time to HH:MM for consistent sorting
      // Google Sheets API may return time as decimal (0.2736 = 6:34 AM) or string ("6:34")
      let pullTime;
      if (typeof rawTime === "number" && rawTime < 1) {
        // Decimal fraction of day → convert to HH:MM
        const totalMin = Math.round(rawTime * 24 * 60);
        const hh = String(Math.floor(totalMin / 60)).padStart(2, "0");
        const mm = String(totalMin % 60).padStart(2, "0");
        pullTime = `${hh}:${mm}`;
      } else {
        const t = String(rawTime || "00:00");
        // Zero-pad if needed: "6:34" → "06:34"
        pullTime = t.includes(":") && t.indexOf(":") < 2 ? "0" + t : t;
        // If still no colon (weird format), pad to ensure sorting
        if (!pullTime.includes(":")) pullTime = "00:00";
      }
