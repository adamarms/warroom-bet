...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({ games, ts: Date.now(), debug: assembled.map(g => ({ id: g.id, away: g.away, home: g.home, aRrl: g.aRrl, hRrl: g.hRrl, ouCount: g.ouData?.length, mlDK: [g.dkMl?.a, g.dkMl?.h] })) });
  } catch (e) {
