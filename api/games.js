...
function engine(g) {
  ...
  // Evaluate RL for BOTH teams, pick the one with better EV
  let rrl = null;
  const rlCandidates = [];
  if (g.aRrl) {
    const pW2 = conv.pAwayWinBy2; const rrlO = g.aRrl; const impl = a2i(rrlO);
    const dec = rrlO > 0 ? 1 + rrlO / 100 : 1 + 100 / (-rrlO); const ev = pW2 * dec - 1;
    if (ev > 0) rlCandidates.push({ pWin: pW2, impl, edge: pW2 - impl, ev, odds: rrlO, rlSide: "away", rlTeam: g.away });
  }
  if (g.hRrl) {
    const pW2 = conv.pHomeWinBy2; const rrlO = g.hRrl; const impl = a2i(rrlO);
    const dec = rrlO > 0 ? 1 + rrlO / 100 : 1 + 100 / (-rrlO); const ev = pW2 * dec - 1;
    if (ev > 0) rlCandidates.push({ pWin: pW2, impl, edge: pW2 - impl, ev, odds: rrlO, rlSide: "home", rlTeam: g.home });
  }
  if (rlCandidates.length > 0) { rlCandidates.sort((a, b) => b.ev - a.ev); rrl = rlCandidates[0]; }
