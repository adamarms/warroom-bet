...
async function fOdds() {
  try {
    const rows = await fSheet("Odds JSON!A2:X20000");
    ...
    rows.forEach(r => {
      ...
      // Use row index as sort key — later rows are always newer (appended by scraper)
      ...
      if (commence) {
        try {
          ...
          // Use row date + approximate time to check if pre-game
          const pullDate = String(r[0]).slice(0, 10);
          const rawTime = r[1];
          let pullHour = 12; // default
