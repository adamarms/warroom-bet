...
async function fOdds() {
  try {
    ...
    rows.forEach(r => {
      ...
      // Track latest row and last pre-game row separately
      if (!latest[uniqueKey] || sortKey > latest[uniqueKey].sortKey) latest[uniqueKey] = { sortKey, row: r };
      // A row is pre-game if it was pulled before the commence time
      if (commence) {
        try {
          const commenceTime = new Date(commence).getTime();
          const pullDateTime = new Date(pullDate + "T" + pullTime + ":00-04:00").getTime();
    ...
    // For each game, use pre-game odds if game has commenced, otherwise latest
    const bestRow = {};
...
async function fWeather(team, gd) {
  ...
  try {
    // Determine local timezone for the stadium
    const tz = s.lon < -115 ? "America/Los_Angeles" : s.lon < -100 ? "America/Denver" : s.lon < -85 ? "America/Chicago" : "America/New_York";
    // Convert commence time to local date and hour
    let d, h = 19;
    if (gd) {
      const gt = new Date(gd);
      // Use the game's ET date from the assembled data, not UTC
      const etMs = gt.getTime() - 4 * 60 * 60 * 1000;
      const etDate = new Date(etMs);
      d = etDate.getUTCFullYear() + "-" + String(etDate.getUTCMonth() + 1).padStart(2, "0") + "-" + String(etDate.getUTCDate()).padStart(2, "0");
      h = etDate.getUTCHours();
    } else {
      d = new Date().toISOString().slice(0, 10);
    }
    const r = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${s.lat}&longitude=${s.lon}&hourly=temperature_2m,wind_speed_10m,wind_direction_10m,precipitation_probability&temperature_unit=fahrenheit&wind_speed_unit=mph&timezone=${tz}&start_date=${d}&end_date=${d}`);
    if (!r.ok) return null; const data = await r.json(); if (!data.hourly) return null;
    // For non-ET timezones, adjust the hour index to local time
    let localH = h;
    if (tz === "America/Los_Angeles") localH = h - 3;
    else if (tz === "America/Denver") localH = h - 2;
    else if (tz === "America/Chicago") localH = h - 1;
    if (localH < 0) localH += 24;
    const i = Math.min(Math.max(localH, 0), (data.hourly.time?.length || 1) - 1);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    res.status(200).json({ games, ts: Date.now(), debug: assembled.map(g => ({ id: g.id, away: g.away, home: g.home, aRrl: g.aRrl, hRrl: g.hRrl, ouCount: g.ouData?.length, mlDK: [g.dkMl?.a, g.dkMl?.h], aSP: g.aSP, hSP: g.hSP, spW_a: g.spW_a, spW_h: g.spW_h, s5_a: g.s5_a, s5_h: g.s5_h, tLW_a: g.tLW_a, tLW_h: g.tLW_h, oLW_a: g.oLW_a, oLW_h: g.oLW_h, pRS_a: g.pRS_a, pRA_a: g.pRA_a, pRS_h: g.pRS_h, pRA_h: g.pRA_h })) });
  } catch (e) {
...
module.exports.debug = async function(req, res) {
  ...
  const SHEET_ID = "1M7xHG_IgczBJULHbabsq4qDJilF2VWetarV_DyuGdDE";
  const SHEET_KEY = "AIzaSyCI4PAjwne4YhcRKHWz17JSTeMLP7h6vMU";
  const u = `https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent("Odds JSON!A2:X5000")}?key=${SHEET_KEY}&valueRenderOption=UNFORMATTED_VALUE`;
  const r = await fetch(u);
  const data = (await r.json()).values || [];
  // Find BOS-CIN rows
  const boscinRows = data.filter(row => String(row[3]).includes("BOS-CIN"));
  const waschcRows = data.filter(row => String(row[3]).includes("WAS-CHC"));
  res.status(200).json({
    bosCin: boscinRows.map(r => ({ gid: r[3], pull: r[1], away: r[5], home: r[6], awRLDK: r[17], hmRLDK: r[18], awRLFD: r[19], hmRLFD: r[20] })),
    wasChc: waschcRows.map(r => ({ gid: r[3], pull: r[1], away: r[5], home: r[6], awRLDK: r[17], hmRLDK: r[18], awRLFD: r[19], hmRLFD: r[20] })),
  });
};
